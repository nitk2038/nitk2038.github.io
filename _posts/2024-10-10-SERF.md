...
### Notations and Problem Definitions with Attribute Filtering
#### Notations
...
- let $A$ be an attributes with specific domain $Dom(A)$
    ...
    - total order(전순서 집합): 정렬 가능한 집합, 아래와 같은 성질을 가진다.
        - 비반사성(irreflexibe): $a < a$ 가 아니다.
        - 비대칭성(asymmetric): $a < b$ 라면 $b < a$가 아니다.
        - 전이성(transitive): 만약 $a < b$ 이고 $b < c$ 라면 $a < c$ 이다.
        - 연결성(connected): 만약 $a \neq b$ 라면 $a < b$ 이거나 $b < a$ 이다.
...
- 이에 따라, key(i.e., $a \in Dom(A)$)가 주어지는 경우, $D$에서 해당 최근접 이웃 벡터를 탐색하는 복잡도는 $O(log(n))$ 이다.

![rfanns_problem_definition](/images/2024-10-10-SeRF/rfanns_problem_definition.png)

- 말이 되게 복잡했지만, 벡터를 정렬하여 이분 탐색을 통해 attribute에 해당되는 범위의 벡터를 탐색하는 복잡도는 $O(log(n))$ 이라는 것이다.

2. Nearest Neighbor Search
3. RFANNS(Range Filtering Approximate Nearest Neighbor Search)
